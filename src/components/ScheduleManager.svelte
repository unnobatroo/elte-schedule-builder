          ...
          {#if editingId === schedule.id}
            ...
            >
              ...
              <button
                type="submit"
                title="Save name"
                aria-label="Save schedule name">✓</button
              >
            </form>
          {:else}
            <button
              ...
              aria-current={schedule.id === activeScheduleId
                ? "true"
                : undefined}
            >
              {schedule.name}
            ...
            <button
              ...
              aria-label={`Rename ${schedule.name}`}>✎</button
            >
            <button
              type="button"
              ...
              title={schedules.length === 1
                ? "You must keep at least one schedule"
                : `Delete ${schedule.name}`}
              aria-label={`Delete ${schedule.name}`}>×</button
            >
          {/if}
        </div>
      {/each}
